...
// var username = require('../common/model.js').username;
...
router.get('/', function(req, res, next) {
    ...
    connection.query('select count(*) as total from users', function(error, results) {
        if (error) {
            ...
            // 4.查询数据
            connection.query('select * from users limit ' + (page.now - 1) * page.every + ',' + page.every, function(error, results, fields) {
                if (error) {
                    console.log('查询失败', error);
                    res.send('服务器内部错误，请联系管理员...');
                } else {
                    // 在views下的users下的index.html
...
router.post('/insert', function(req, res) {
    ...
    // console.log(req.body.password); //  e807f1fcf82d132f9bb018ca6738a19f

    // 时间在数据保存的时候，使用的是时间戳 - 从1970年1月1日到现在的秒数
    req.body.addtime = Math.floor(new Date().getTime() / 1000);

    // 4.将数据添加到数据库
    connection.query('insert into users(username,age,sex,password,phone,addtime) values("' + req.body.username + '",' + req.body.age + ',' + req.body.sex + ',"' + req.body.password + '","' + req.body.phone + '",' + req.body.addtime + ')', function(error, results, fileds) {
        if (error) {
...
router.get('/image/:id', function(req, res) {
    ...
    res.render('users/image', { id: req.params.id, admin: req.session.admin });
})
...
router.post('/image', upload.single('photo'), function(req, res) {
    ...
    fs.rename(oldname, newname, function(error) {
        if (error) {
            ...
            // 将名字保存到数据库
            connection.query('update users set photo="' + filename + '" where id=' + req.body.id, function(error) {
                if (error) {
...
router.post('/ainsert',function(req,res){
    ...
    connection.query('insert into users(username,password) values("' + req.body.username + '","' + req.body.password + '")',function(error,results){
        if (error) {
            console.log(error);
            res.json({success:0});
        } else {
...
// router.post('/aup',function(req,res){
//     req.body.addtime=Math.floor(new Date().getTime()/1000);
//     connection.query('update users set username="' + req.body.username + '",password="'+req.body.password+'" where id='+req.body.id+'',function(error,results){
//         if(error){
//             console.log(error);
//             res.json({success:0});
//         }else {
...
router.post('/adelete',function(req,res){
    // console.log(req.body);
    connection.query('delete from users where id = '+req.body.id, function(error){
        if (error) {
            // 删除失败
            res.json({success:0});
        } else {
